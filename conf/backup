...
for application in $(sudo ls /etc/yunohost/apps/); do
    ...
    if sudo test ! -f "/etc/yunohost/apps/$application/scripts/backup"; then
        echo "[WARN] The application $application has no backup script. This app won't be backed up."
        continue
    fi

    apps+=("$application")
    echo "- App '$application' added'"
done
...
if $is_verbose; then
    echo "yunohost backup create --name '${current_date}___APP__' --method '__APP___app' $option_system $apps_option"
fi

sudo yunohost backup create --name "${current_date}___APP__" --method "__APP___app" "$option_system" "$apps_option"
exit $?